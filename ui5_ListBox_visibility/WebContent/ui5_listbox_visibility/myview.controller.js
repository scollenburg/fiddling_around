		...
		.controller(
				"ui5_listbox_visibility.myview",
				{
					...
					onAfterRendering : function() {
						...
						/*
						 * Nice in theory, however this does *not* get called
						 * during rerendering
						 */
						...
						if (!this.oHidingHandler) {
							var o = {
								onAfterRendering : function(oEvent) {
									...
									var bFixIt = this.getModel().getProperty(
											"/uistate/enableWorkaround");

									var oSAPDom = this.$();
									/*
									 * We can't query using ID, since it will
									 * promptly be turned into a
									 * document.getElementById() call, which
									 * will only return the first element found
									 * (since ID's are not supposed to be
									 * duplicated)
									 */
